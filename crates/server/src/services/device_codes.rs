...
impl DeviceCodeService {
    ...
    /// CAS consume an approved row and mint its tokens in one transaction.
    /// Strict: returns true only when the row was `approved` (affected==1);
    /// a failed mint leaves it approved for the next poll.
    pub async fn consume_and_issue_tokens(
        &self,
        device_code: &str,
        ...
        self.store
            .consume_device_code_and_issue_tokens(device_code, access, refresh)
            .await
    }
